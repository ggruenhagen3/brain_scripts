...
total_neg_genes_per_cluster <- rep(0, num_clusters+1)
for (run in 1:num_run) {
  ...
  for (i in 0:num_clusters) {
    ...
    neg_genes_per_cluster <- c(neg_genes_per_cluster, length(which(as.vector(mat[,this_cells]) == 0))) # genes
    this_trans <- c(this_trans, sum(rowSums(as.matrix(mat[,this_cells]))))
    cells_per_cluster <- c(cells_per_cluster, length(this_cells))
  }
  total_neg_genes_per_cluster <- total_neg_genes_per_cluster + neg_genes_per_cluster
...
results <- data.frame()
for (i in 0:num_clusters) {
  cluster <- c(avg_genes_per_cluster[i+1],avg_neg_genes_per_cluster[i+1])
  contig_table <- data.frame(cluster, all)
...
print(all[1]/all[2])
write.table(results, file = paste(rna_path, "/results/down_and_fisher_2_", bio, ".tsv", sep=""), sep = "\t", row.names = FALSE, quote=FALSE)