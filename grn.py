...
def main():
    ...
    for i in range(0, num_perm):
        ...
        with multiprocessing.Pool(2) as pool:
            ...
            num_cells_bhve = len(np.array(data_mat[:, mat_idx['B' + str(i)]].astype(bool).sum(axis=1))[:, 0])
            num_cells_ctrl = len(np.array(data_mat[:, mat_idx['C' + str(i)]].astype(bool).sum(axis=1))[:, 0])
