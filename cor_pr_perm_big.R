...
# Constants
output_folder = "~/scratch/brain/results/py_ns_ieg5/"
n_perm = 10000 # 200 matrices total because bhve and ctrl
n_perm_per_job = 500
pbs_file_name = "~/scratch/brain/brain_scripts/py_ns_ieg5.pbs"
...
sapply(1:(n_perm/n_perm_per_job), function(x) {
  ...
  writeLines(c("#PBS -A GT-js585-biocluster",
               paste0("#PBS -N grn_ieg5_", x),
               "#PBS -l mem=128gb",
               "#PBS -l nodes=2:ppn=4",
               "#PBS -l walltime=90:00:00",
               "#PBS -j oe",
               paste0("#PBS -o grn_ieg5_", x, ".out"),
               "#PBS -m abe",
               ...
               "module load r",
               "conda activate r4\n",
               paste("python grn.py", x, n_perm_per_job, "-a -i 5")),
             fileConn)
  close(fileConn)
