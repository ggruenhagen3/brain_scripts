# Read Input ===================================================================
# boot_num = 1; min_dist = 0.1; n_neighbors = 10; res = 0.2; nboot = 1;
args = commandArgs(trailingOnly=TRUE)
boot_num = as.numeric(args[1])
min_dist = as.numeric(args[2])
n_neighbors = as.numeric(args[3])
nboot = as.numeric(args[4])
num_cores = 10
if (length(args) > 5) { num_cores = as.numeric(args[5])}
boot.pct = 0.8
message(paste0("Running w/ Parameters: boot_num=", boot_num, ", min_dist=", min_dist, ", n_neighbors=", n_neighbors, ", nboot=", nboot, ", num_cores=", num_cores))
...
sc.cluster.boot = function(x) {
  message(paste0("Starting Boot=", x))
  message("Subsampling")
...
for (res in resolutions) {
  ...
  cluster.jaccard.list = mclapply(1:nboot, function(x) evaluate.boot(x), mc.cores = num_cores)
  ...
  elapsed.time = round(elapsed.time["elapsed"])
  message(paste0(" - - - Finished Resolution=", res, " in ", elapsed.time, "s (George) - - - "))
}
message("All Done")
