...
for (run in 1:50) {
  ...
  for (i in 0:num_clusters) {
    ...
    genes_per_cluster <- c(genes_per_cluster, sum(rowSums(as.matrix(mat[,this_cells])))) # genes
...
print(all[1]/all[2])
write.table(results, file = paste(rna_path, "/results/down_and_fisher_trans.tsv", sep=""), sep = "\t", row.names = FALSE, quote=FALSE)